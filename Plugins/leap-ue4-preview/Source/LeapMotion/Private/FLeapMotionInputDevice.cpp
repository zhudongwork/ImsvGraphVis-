...
{
	UE_LOG(LeapMotionLog, Log, TEXT("OnConnect."));
...
void FLeapMotionInputDevice::OnDeviceFound(const LEAP_DEVICE_INFO *props)
{
	...
	UE_LOG(LeapMotionLog, Log, TEXT("OnDeviceFound %s."), *Stats.DeviceInfo.PID);
}

void FLeapMotionInputDevice::OnFrame(const LEAP_TRACKING_EVENT *frame)
{
	//Current version polls for frames just before they're needed (lower latency)

	/* Disabled, TODO: remove completely
	FLeapLambdaRunnable::RunShortLambdaOnGameThread([this, frame]
	{
	...
	MessageHandler(InMessageHandler)
{
	...
	SlerpTween = 0.5f;
	GameTimeInSec = 0.f;
	HMDType = EHMDDeviceType::DT_SteamVR;
	...
	Config.DeviceName = "LeapMotion";
	Config.InputType = EBodyStateDeviceInputType::CAMERA_MOUNTED_INPUT_TYPE;
	BodyStateDeviceId = UBodyStateBPLibrary::DeviceAttachedRaw(Config, this);
...
void FLeapMotionInputDevice::ParseEvents()
{
	...
	{
		CurrentFrame.TranslateFrame(Options.HMDOffset);		//Offset HMD-Leap
...
void FLeapMotionInputDevice::ShutdownLeap()
{
	...
	UBodyStateBPLibrary::DeviceDetached(BodyStateDeviceId);
...
void FLeapMotionInputDevice::UpdateInput(int32 DeviceID, class UBodyStateSkeleton* Skeleton)
{
	...
	//Reset Data
	auto LeftArmPre = Skeleton->LeftArm();
	LeftArmPre->LowerArm->BoneData.Confidence = 0.f;
	auto RightArmPre = Skeleton->RightArm();
	RightArmPre->LowerArm->BoneData.Confidence = 0.f;
	...
	for (auto LeapHand : CurrentFrame.Hands)
	{
		if (LeapHand.HandType == EHandType::LEAP_HAND_LEFT)
		{
			...
			//We're tracking that hand, show it
			LeftArm->LowerArm->BoneData.Confidence = 1.f;
			LeftArm->LowerArm->Meta.ParentDistinctMeta = true;
		}
		else if (LeapHand.HandType == EHandType::LEAP_HAND_RIGHT)
		{
			auto RightArm = Skeleton->RightArm();
			...
			//We're tracking that hand, show it
			RightArm->LowerArm->BoneData.Confidence = 1.f;
			RightArm->LowerArm->Meta.ParentDistinctMeta = true;
		}
	}
}
...
void FLeapMotionInputDevice::SetBSHandFromLeapHand(UBodyStateHand* Hand, const FLeapHandData& LeapHand)
{
	...
	Hand->Wrist->BoneData.Confidence = LeapHand.Confidence;
}
...
void FLeapMotionInputDevice::SetOptions(const FLeapOptions& InOptions)
{
	if (GEngine && GEngine->HMDDevice.IsValid())
	{
		HMDType = GEngine->HMDDevice->GetHMDDeviceType();
	}
	else
	{
		Options.Mode = ELeapMode::LEAP_MODE_DESKTOP;
	...
	else
	{
		...
		if (HMDType == EHMDDeviceType::DT_SteamVR ||
			HMDType == EHMDDeviceType::DT_GearVR)
		{
			FVector ViveOffset = FVector(9.0, 0, 0);
			Options.HMDOffset = ViveOffset;
		...
		else if (HMDType == EHMDDeviceType::DT_OculusRift)
		{
			FVector OculusOffset = FVector(8.0, 0, 0);
			Options.HMDOffset = OculusOffset;
		...
		else if (HMDType == EHMDDeviceType::DT_GoogleVR)
		{
			FVector ViveOffset = FVector(8.0, 0, 0);
